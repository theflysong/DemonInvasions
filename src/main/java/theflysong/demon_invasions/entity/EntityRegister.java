...
import theflysong.demon_invasions.util.Constructor1;
import theflysong.demon_invasions.util.ProtectedHelper;
...
public class EntityRegister {
    ...
    public final DeferredRegister<EntityType<?>> entityRegister = DeferredRegister.create(ForgeRegistries.ENTITIES, DemonInvasions.MODID);
    ...
    public class RegistryEntry<E extends Entity> {
        ...
        public RegistryEntry<E> entity(Supplier<EntityType.Builder<E>> sup) {
            typeObject = entityRegister.register(name, () -> {
                ...
                if (this.egg == null) {
                    DemonInvasions.getLogger().warn("No egg for " + name);
                } else {
                    assert EGGS != null;
